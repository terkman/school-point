...
export function AdminDashboard({ account, state, onChange, onResetDemo, onLogout }: AdminDashboardProps) {
  ...
  function decideAppeal(appealId: string, accepted: boolean) {
    ...
    if (!appeal || !source || !student || !['submitted', 'reviewing'].includes(appeal.status)) return
    if (!accepted) {
      onChange({
        ...state,
  ...
  function addPointsDirectly(event: FormEvent<HTMLFormElement>) {
      ...
      {tab === 'manage' ? (
        <div className="manage-grid">
          <form className="panel stack-form" onSubmit={addPointsDirectly}><div className="section-heading"><div><p className="eyebrow">สิทธิ์ผู้ดูแลระบบ</p><h2>เพิ่มคะแนนโดยตรง</h2></div></div>
            ...
            <button className="button primary" type="submit">เพิ่มคะแนนและบันทึกประวัติ</button>
          </form>
          <section className="panel"><div className="section-heading"><div><p className="eyebrow">ภาคเรียน</p><h2>เริ่มคะแนนที่ 100</h2></div></div><p>รายการคะแนนเดิมยังคงอยู่ เคสติดตามที่ไม่เสร็จจะยกไปต่อโดยไม่ยกคะแนนติดลบ</p><div className="reset-preview"><span>นักเรียนที่จะรีเซ็ต <strong>{state.students.length}</strong></span><span>เคสที่จะคงไว้ <strong>{openCases.length}</strong></span></div><button className="button warning full" disabled={Boolean(state.term.resetCompletedAt)} onClick={resetTermScores}>{state.term.resetCompletedAt ? `รีเซ็ตแล้ว ${formatThaiDate(state.term.resetCompletedAt)}` : 'ตรวจสอบและรีเซ็ตคะแนน'}</button></section>
          ...
          <section className="panel danger-zone"><div className="section-heading"><div><p className="eyebrow">สำหรับการทดสอบ</p><h2>คืนค่าข้อมูลสาธิต</h2></div></div><p>ล้างเฉพาะข้อมูลสมมติในเบราว์เซอร์นี้ ไม่มีผลต่อฐานข้อมูลจริง</p><button className="button reject" onClick={onResetDemo}>คืนค่าข้อมูลตัวอย่าง</button></section>
        </div>
      ) : null}
    </AppShell>
  )
