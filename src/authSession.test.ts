...
describe('sessionUserId', () => {
  it('preserves the school-data dependency after hidden to visible produces same-user SIGNED_IN', () => {
  ...
  it('preserves the school-data dependency after hidden to visible refreshes the same user token', () => {
  ...
  it('returns the current signed-in identity', () => {
    expect(sessionUserId(null)).toBeUndefined()
    expect(sessionUserId({ user: { id: 'student-2' } } as Session)).toBe('student-2')
  })

  it('changes the school-data dependency when a different user signs in', () => {
  ...
  it('keeps USER_UPDATED information from reloading same-user school data', () => {
    const currentSession = { user: { id: 'student-1' } } as Session
  ...
  it('changes the school-data dependency when the user signs out', () => {
    const currentSession = { user: { id: 'student-1' } } as Session
    expect(sessionUserId(null)).toBeUndefined()
